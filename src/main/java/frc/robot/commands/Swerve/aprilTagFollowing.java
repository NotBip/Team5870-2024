...
public class aprilTagFollowing extends Command {
    ...
    public aprilTagFollowing(SwerveSubsystem swerveSubsystem, PhotonLL limelight) { 
    ...
    public void execute() {
        ...
        double velStrafe = 0;
        double velRot = 0;
        double driveOffset = 2; 
        double strafeOffset = 0; 
        double rotOffset = 180;

        velForward = drivePID.calculate(limelight.getXDistance(), driveOffset);
        velStrafe = strafePID.calculate(limelight.getYDistance(), strafeOffset);
